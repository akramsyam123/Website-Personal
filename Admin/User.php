  ...
  <td height="26">
    <form id="form1" name="form1" method="post" action="">
      <table width="100%" border="0" cellpadding="0" cellspacing="0">
        <tr>
          <td height="32">User Name (Search or Add):</td>
          <td>
            <label>
              <input type="text" name="txtUserName" id="txtUserName" value="<?php echo isset($_POST['txtUserName']) ? $_POST['txtUserName'] : ''; ?>" />
            </label>
          </td>
          <td>
            <label>
  ...
  <td>
    <table width="100%" border="1" bordercolor="#003300">
      ...
      if (isset($_POST['addButton'])) {
          ...
          $insertQuery = "INSERT INTO admin_master (UserName, Password) VALUES ('$username', '$password')";
          mysqli_query($con, $insertQuery);
          echo "<p>User added successfully!</p>";
      }

      // Retrieve data logic
      $searchQuery = isset($_POST['txtUserName']) ? $_POST['txtUserName'] : '';
      if (!empty($searchQuery) && isset($_POST['searchButton'])) {
          // Search by Username
          $sql = "SELECT * FROM admin_master WHERE AdminId LIKE '%" . mysqli_real_escape_string($con, $searchQuery) . "%' OR UserName LIKE '%" . mysqli_real_escape_string($con, $searchQuery) . "%'";
      } else {
